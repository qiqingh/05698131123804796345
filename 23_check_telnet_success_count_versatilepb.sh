...
if [[ -z $8 ]]; then 
    echo "Usage: $0 <path-to-login.log> <sleep seconds> <wan-interface> <cloud_platform> <image> <rootfs> <module-path> <dtb-path>"
    exit 1
fi

new_count=$(grep "root login on" "$1" | wc -l)

old_count=$(cat telnet_success_count_23.txt)
...
if test $new_count -gt $old_count; then 
    # echo $new_count > telnet_success_count_23.txt 
    echo "[23_qemu]: New telnet login occurs, going to reboot ... "
    echo "[23_qemu]: Sleeping for $2 seconds"
    echo "$2" > sleep_duration_23.txt
    sleep $2
    #echo "[23_qemu]: Backing up logs ... "
    #log_path=`echo $1 | rev | cut -d "/" -f 3- | rev`
    #cp -r "$log_path" "$log_path_telnet"
    echo "[23_qemu]: Going to relaunch ... "
	# Clean up any old qemu (kill screen sessions and qemu)
	./33_clean_guest.sh "$3" 192.168.1.1
	sleep 1
    echo "[23_qemu]: Cleaning old rootfs image ... "
    ./24_retrive_new_rootfs.sh "$6"
    sleep 1

    # Kill zeek 
    echo "[23_qemu]: Terminating zeek  ... "
    ./97_stop_zeek.sh 1
    # kill tcpdump
    echo "[23_qemu]: Terminating tcpdump ... "
    ./95_stop_tcpdump_with_name.sh 1
    # kill log parsers
    echo "[23_qemu]: Terminating log parsers ... "
