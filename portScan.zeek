...
#            Seth Hall
#            All the authors of the old scan.
...
event zeek_init() &priority=5
	...
	SumStats::create([$name="addr-scan",
	                  ...
	                  $threshold_crossed(key: SumStats::Key, result: SumStats::Result) =
	                  	...
	                  	local message=fmt("[prosec_host_scanned]: %s scanned at least %d unique hosts on port %s in %s", key$host, r$unique, key$str, dur);
	...
	SumStats::create([$name="port-scan",
	                  ...
	                  $threshold_crossed(key: SumStats::Key, result: SumStats::Result) =
	                  	...
	                  	local side = Site::is_local_addr(key$host) ? "local" : "remote";
	                  	local dur = duration_to_mins_secs(r$end-r$begin);
	                  	local message = fmt("[prosec_port_scaned]: %s scanned at least %d unique ports of host %s in %s", key$host, r$unique, key$str, dur);
